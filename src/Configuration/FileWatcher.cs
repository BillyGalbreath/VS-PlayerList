...
public class FileWatcher {
    ...
    public FileWatcher() {
        ...
        _watcher.Filter = Config.ConfigFile;
    ...
    private void Error(object sender, ErrorEventArgs e) {
        PlayerListMod.Instance.Mod.Logger.Error(e.GetException().ToString());
        QueueReload();
    }
    ...
    private void QueueReload(WatcherChangeTypes? change = null) {
        ...
        PlayerListMod.Instance.Api?.Event.RegisterCallback(_ => {
            if (!string.IsNullOrEmpty(changeType)) {
                PlayerListMod.Instance.Mod.Logger.Event($"Detected the config was {changeType}");
            }

            Config.Reload();

            // wait 100ms more to remove this change from the queue since the reload triggers another write
            PlayerListMod.Instance.Api.Event.RegisterCallback(_ => _queue.TryRemove(changeType, out bool _), 100);
        }, 100);
    }
